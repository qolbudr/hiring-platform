...
export async function GET(req: Request) {
  try {
    ...
    const jobCollection = await getDocs(query(collection(db, "jobs"), where("title", ">=", search), where("title", "<=", search + "\uf8ff")));
    const jobs: Job[] = jobCollection.docs.map(doc => Job.parse({ id: doc.id, ...doc.data() }));
